...
#SIGN OFF NOTES 11/14/19 2:00am
...
# * Must begin implementation of legitimate AI combat system, focusing on combat, searching and idle behaviours.
# ^> The inclusion of some rudimentary faculties has been achieved. The AIs are now capable of performing weapon-firing initation and cessation, and multiple necessary attributes have been added to the AI in order
#to facilitate the necessary calculations based on line-of-sight, as the previously determined data was largely abandoned after discovery. It has now been utilized for further combat techniques and decision-making.
#A method for bypassing obstacles obfuscating line of sight on a target has been determined, and is still in beta-testing. The way-points around these obstacles now have a creation-method that guarantees no object-
#overlap, but a methodology for determining the appropriate way-points for travel must now be included.
# *11/12/19-9:30am-The obstacle-path nodes now have proper field of views that can allocate the correct bounding_points of the target unit, all that remains is to perform calculations to determine LOS by repurposing
#the original LOS calculations to be applicable to the nodes and not just the Artificial FOV deltoid.
# *T^>The EnemyAI class has been renamed to Artificial, to better represent the current social climate.
# * 11/13/19-1:30am-The obstacle-path nodes have received significant development in regards to the completion of true LOS checking, which I expect to be completed in short order. I am, however, too exhausted to
#trust myself to write code worthy of Salvo!, and thus shall leave it to a future iteration of myself whom has either had sufficient sleep or caffeine of equal calibre.
# * 11/14/19-2:00am-The obstacle-path nodes have finally achieved a resounding failure at adequately assessing target-sight! Huzzah! It would appear that, due to the significantly smaller size of the FOV-deltoids
#generated by the obstacle-nodes, they are not sufficiently wide enough to achieve the necessary field-of-view in order to capture any obstacle bounding-points that would otherwise indicate obstructed line-of-sight.
#A solution should be relatively simple to achieve, however, by drawing a line from the clockwise/counterclock LOS points of the potentially obstructing obstacle and determining if the FOV lines intersect with that
#line (note that this will not utilize legitimate line-intersection calculations, as this is simply a figure of speech.) They should simply be able to calculate whether the requisite degrees are within the boundaries
#of the obstacle's degrees, though if I'm not mistaken that is what is already being achieved. Perhaps there is some error in calculating the points. Either way, this is a problem that will require a less exhausted me.
# ^> The solution is so obvious in hindsight! The mistake was not to check the degrees of the fov-deltoid against the 'fov-deltoid' of the (potentially) obstructing units! It's currently only happening in
#a sole fashion, and instead needs a reciprocal component in order to function correctly.
...
#            line_x = create_line(self.fov_deltoid['origin']['pos'], positions[x])
    ...
    def check_unit_sighted_fov(self, unit_id, fov_unit_stats_omni, fov_unit_keys):
        '''
    ...
    def determine_obstacle_nodes(self, unit_id):
        ...
        for x in range(len(obscuring_units)):
            ...
            for i in range(len(self.master_sector.units[obscuring_units[x]]['bounding_points'])):
                ...
                new_bounding_i = pos_from_degrees(self.master_sector.units[obscuring_units[x]]['unit_rect'].center, bounding_i_angle, (bounding_i_dist + int(self.tank.node_distance / 2)))
            ...
            for i in range(len(obscuring_units_path_points[obscuring_units[x]]['nodes'])):
                if obscuring_units_path_points[obscuring_units[x]]['nodes'][i] != None:
                    ...
                    node_stats = {'clockwise_index': clock_conn, 'counterclock_index': counter_conn, 'node_fov': node_fov, 'target_sight': None}
    ...
    def determine_obstacle_node_path(self, unit_id):
        '''
        Calculates the path, from node-to-node, towards a specified target-unit that is obscured by one or more obstacles.
        '''
        ...
        for x in range(len(obscuring_units_path_points['unit_keys'])):
            ...
            for i in range(len(obscuring_units_path_points[unit_key_x]['nodes'])):
                if obscuring_units_path_points[unit_key_x]['nodes'][i] != None:
                    ...
                    obscuring_units_path_points[unit_key_x]['node_stats'][obscuring_units_path_points[unit_key_x]['nodes'][i]]['target_sight'] = self.check_unit_sighted_fov(unit_id, fov_unit_stats_omni, fov_unit_keys_i)
        for x in range(len(obscuring_units_path_points['unit_keys'])):
            unit_key_x = obscuring_units_path_points['unit_keys'][x]
            for i in range(len(obscuring_units_path_points[unit_key_x]['nodes'])):
