...
class MasterSector():
    ...
    def bounding_box_collision_check(self, box_a_stats, box_b_stats):
        ...
        If no points are found to be inhabiting either box, a final check to determine the occurrence of a pointless-overlap-collision is performed.
        This determines the appropriate cross-line for each bounding-box (the line which runs from one point to its opposite point, to which it has no connecting bounding-lines)
        and then performs an intersection check with these two cross-lines. If an intersection is identified, this indicates that, while no bounding-point from one box inhabits the
        other, an overlap (and thus a collision) is present. 
        ...
        for x in range(len(box_a_active_points)):
            ...
            if normalized_b_x_chosen_zero_diff < 2.5:
                ...
                if normalized_b_x_deg > box_b_zero_degs[1]:
                    ...
                    box_a_points_alt_bounding_indexes.append((0, 1))
            pygame.draw.circle(DISPSURF, (0, 255, 255), box_a_active_points[x], 10, 3)
        ...
        for x in range(len(box_b_active_points)):
            ...
            if normalized_a_x_chosen_zero_diff < 2.5:
                ...
                if normalized_a_x_deg > box_a_zero_degs[1]:
                    ...
                    box_b_points_alt_bounding_indexes.append((0, 1))
            pygame.draw.circle(DISPSURF, (255, 0, 255), box_b_active_points[x], 10, 3)
        ...
        if len(box_b_point_collisions) > 0:
            ...
            box_a_centers_line_box_b_cross_inter = line_segment_intersect_bool(box_a_points[box_a_b_perp_indexes[0]], box_a_points[box_a_b_perp_indexes[1]], box_b_points[box_b_a_perp_indexes[0]], box_b_points[box_b_a_perp_indexes[1]])
            if box_a_centers_line_box_b_cross_inter == True:
                bounding_collision_b = [box_a_collision_rejects[x] for x in range(len(box_a_collision_rejects)) if box_a_collision_rejects[x][1] == box_a_points[box_a_b_perp_host_index]][0]#in [box_a_points[box_b_a_perp_indexes[0]], box_a_points[box_b_a_perp_indexes[1]]]]#== [box_a_points[box_a_b_perp_host_index]]
                bounding_collision_a = [box_b_collision_rejects[x] for x in range(len(box_b_collision_rejects)) if box_b_collision_rejects[x][1] == box_b_points[box_b_a_perp_host_index]][0]#in [box_b_points[box_a_b_perp_indexes[0]], box_b_points[box_b_a_perp_indexes[1]]]]#== box_b_points[box_b_a_perp_host_index]]

        #######################################
        #BOUNDING COLLISION EXPANSION TESTING AREA
        #######################################
        pygame.draw.aaline(DISPSURF, (0, 0, 255), box_a_points[0], box_a_points[1], True)
        pygame.draw.aaline(DISPSURF, (0, 0, 255), box_a_points[1], box_a_points[3], True)
        pygame.draw.aaline(DISPSURF, (0, 0, 255), box_a_points[3], box_a_points[2], True)
        pygame.draw.aaline(DISPSURF, (0, 0, 255), box_a_points[2], box_a_points[0], True)


        box_a_line_pair_halfway_fore = pos_from_degrees(box_a_points[0], angle_between_points(box_a_points[1], box_a_points[0]), distance_between_positions(box_a_points[0], box_a_points[1]) / 2)
        box_a_line_pair_halfway_flank = pos_from_degrees(box_a_points[0], angle_between_points(box_a_points[2], box_a_points[0]), distance_between_positions(box_a_points[0], box_a_points[2]) / 2)
        box_a_halfway_fore_dist = distance_between_positions(vec2int(box_a_line_pair_halfway_fore), box_a_center)
        box_a_halfway_flank_dist = distance_between_positions(vec2int(box_a_line_pair_halfway_flank), box_a_center)

        pygame.draw.circle(DISPSURF, (255, 0, 0), vec2int(box_a_line_pair_halfway_fore), 3, 0)
        pygame.draw.circle(DISPSURF, (0, 255, 0), vec2int(box_a_line_pair_halfway_flank), 3, 0)
        pygame.draw.polygon(DISPSURF, (255, 0, 0), [box_a_center, vec2int(box_a_line_pair_halfway_fore)], 2)
        pygame.draw.polygon(DISPSURF, (0, 255, 0), [box_a_center, vec2int(box_a_line_pair_halfway_flank)], 2)
        pygame.draw.circle(DISPSURF, (255, 0, 0), box_a_center, box_a_halfway_fore_dist, 1)
        pygame.draw.circle(DISPSURF, (0, 255, 0), box_a_center, box_a_halfway_flank_dist, 1)

        box_a_halfway_flank_perp_point = vec2int(pos_from_degrees(box_a_center, box_b_perp_deg, box_a_halfway_flank_dist))
        box_a_halfway_fore_perp_point = vec2int(pos_from_degrees(box_a_center, box_b_perp_deg, box_a_halfway_fore_dist))

        box_a_flank_radial_intersect = line_segment_intersect_bool(box_a_center, box_a_halfway_flank_perp_point, box_b_points[box_b_bounding_indexes[0]], box_b_points[box_b_bounding_indexes[1]], on_seg=False)
        box_a_fore_radial_intersect = line_segment_intersect_bool(box_a_center, box_a_halfway_fore_perp_point, box_b_points[box_b_bounding_indexes[0]], box_b_points[box_b_bounding_indexes[1]], on_seg=False)


        box_a_flank_radial_point_coll = False
        box_a_fore_radial_point_coll = False

        box_a_fore_radial_rect = pygame.Rect((box_a_center[0] - (box_a_halfway_fore_dist)), (box_a_center[1] - (box_a_halfway_fore_dist)), box_a_halfway_fore_dist * 2, box_a_halfway_fore_dist * 2)
        box_a_flank_radial_rect = pygame.Rect((box_a_center[0] - (box_a_halfway_flank_dist)), (box_a_center[1] - (box_a_halfway_flank_dist)), box_a_halfway_flank_dist * 2, box_a_halfway_flank_dist * 2)
        box_a_fore_radial_box_b_bounding_colls = [(bool(box_a_fore_radial_rect.collidepoint(box_b_points[box_b_bounding_indexes[0]])), 0), (bool(box_a_fore_radial_rect.collidepoint(box_b_points[box_b_bounding_indexes[1]])), 1)]
        box_a_fore_radial_box_b_coll_result = sorted([box_a_fore_radial_box_b_bounding_colls[x][0] for x in range(len(box_a_fore_radial_box_b_bounding_colls))])[-1]
        box_a_fore_radial_box_b_coll_point = sorted([box_a_fore_radial_box_b_bounding_colls[x][1] if box_a_fore_radial_box_b_bounding_colls[x][0] == True else 0 for x in range(len(box_a_fore_radial_box_b_bounding_colls))])[-1]
        box_a_flank_radial_box_b_bounding_colls = [(bool(box_a_flank_radial_rect.collidepoint(box_b_points[box_b_bounding_indexes[0]])), 0), (bool(box_a_flank_radial_rect.collidepoint(box_b_points[box_b_bounding_indexes[1]])), 1)]
        box_a_flank_radial_box_b_coll_result = sorted([box_a_flank_radial_box_b_bounding_colls[x][0] for x in range(len(box_a_flank_radial_box_b_bounding_colls))])[-1]
        box_a_flank_radial_box_b_coll_point = sorted([box_a_flank_radial_box_b_bounding_colls[x][1] if box_a_flank_radial_box_b_bounding_colls[x][0] == True else 0 for x in range(len(box_a_flank_radial_box_b_bounding_colls))])[-1]
        
        #pygame.draw.rect(DISPSURF, (0, 255, 0), box_a_fore_radial_rect, 1)
        #pygame.draw.rect(DISPSURF, (255, 0, 0), box_a_flank_radial_rect, 1)

        if box_a_flank_radial_intersect == False and box_a_flank_radial_box_b_coll_result == True:
            bounding_collision_a_point_dist = distance_between_positions(box_a_center, box_b_points[box_b_bounding_indexes[box_a_flank_radial_box_b_coll_point]])
            if bounding_collision_a_point_dist < box_a_halfway_flank_dist:
                box_a_flank_radial_point_coll = True
                box_a_flank_radial_box_b_coll_point = box_b_points[box_b_bounding_indexes[box_a_flank_radial_box_b_coll_point]]
        if box_a_fore_radial_intersect == False and box_a_fore_radial_box_b_coll_result == True:
            bounding_collision_a_point_dist = distance_between_positions(box_a_center, box_b_points[box_b_bounding_indexes[box_a_fore_radial_box_b_coll_point]])
            if bounding_collision_a_point_dist < box_a_halfway_fore_dist:
                box_a_fore_radial_point_coll = True
                box_a_fore_radial_box_b_coll_point = box_b_points[box_b_bounding_indexes[box_a_fore_radial_box_b_coll_point]]

        if box_a_flank_radial_intersect == True or box_a_flank_radial_point_coll == True:
            pygame.draw.circle(DISPSURF, (255, 0, 0), box_a_center, box_a_halfway_flank_dist, 5)
        if box_a_fore_radial_intersect == True or box_a_fore_radial_point_coll == True:
            pygame.draw.circle(DISPSURF, (0, 255, 0), box_a_center, box_a_halfway_fore_dist, 5)

        if box_a_flank_radial_intersect == True:
            pygame.draw.circle(DISPSURF, (255, 0, 0), box_b_points[box_b_bounding_indexes[0]], 10, 3)
            pygame.draw.circle(DISPSURF, (0, 255, 0), box_b_points[box_b_bounding_indexes[1]], 10, 3)
            box_a_box_b_point_dist = distance_between_positions(box_a_center, box_b_points[box_b_bounding_indexes[0]])
            angle_a = abs(get_degree_diff(angle_between_points(box_b_points[box_b_bounding_indexes[1]], box_b_points[box_b_bounding_indexes[0]]), angle_between_points(box_a_center, box_b_points[box_b_bounding_indexes[0]])))
            angle_c = abs(get_degree_diff(angle_between_points(box_b_points[box_b_bounding_indexes[0]], box_b_points[box_b_bounding_indexes[1]]), polarize_degree(box_b_perp_deg)))
            side_a = box_a_box_b_point_dist * math.sin(math.radians(angle_a)) / math.sin(math.radians(angle_c))
            generated_point = vec2int(pos_from_degrees(box_a_center, box_b_perp_deg, side_a))
            pygame.draw.polygon(DISPSURF, (86, 0, 255), [box_a_center, generated_point], 3)
            pygame.draw.circle(DISPSURF, (86, 0, 255), generated_point, 3, 0)
            #new_player_tank_pos = vec2int(pos_from_degrees(generated_point, polarize_degree(box_b_perp_deg), box_a_halfway_flank_dist))
            playerTank_unit_id = [list(self.units.keys())[x] for x in range(len(list(self.units.keys()))) if self.units[list(self.units.keys())[x]]['allegiance'] == 'PLAYER'][0]
            chassis_velocity = pygame.math.Vector2(1, 0).rotate(polarize_degree(box_b_perp_deg)) * (box_a_halfway_flank_dist - side_a)
            self.unit_objects[playerTank_unit_id].chassis_pos = vec2int(self.unit_objects[playerTank_unit_id].chassis_pos + chassis_velocity)
        
        elif box_a_flank_radial_point_coll == True:
            box_a_box_b_point_dist = distance_between_positions(box_a_center, box_a_flank_radial_box_b_coll_point)
            box_b_bounding_coll_box_a_center_deg = angle_between_points(box_a_center, box_a_flank_radial_box_b_coll_point)
            playerTank_unit_id = [list(self.units.keys())[x] for x in range(len(list(self.units.keys()))) if self.units[list(self.units.keys())[x]]['allegiance'] == 'PLAYER'][0]
            chassis_velocity = pygame.math.Vector2(1, 0).rotate(polarize_degree(box_b_bounding_coll_box_a_center_deg)) * (box_a_halfway_flank_dist - box_a_box_b_point_dist)
            self.unit_objects[playerTank_unit_id].chassis_pos = vec2int(self.unit_objects[playerTank_unit_id].chassis_pos + chassis_velocity)
        ...
        #######################################
        #END BOUNDING COLLISION EXPANSION TESTING AREA
        #######################################
...
class Artificial():
    ...
    def turn_chassis_to_desired(self):
        if self.movement_stats['pathfinding']['path_found'] == False:
    ...
    def move_chassis_to_goal(self):
        '''
        Engages repositioning for the Artificial's unit, following the Artificial's movement_stats['pathfinding']['path'] attribute node-by-node until the goal_node is successfully reached.
        '''
        
        if self.movement_stats['pathfinding']['path_found'] == False or self.movement_stats['pathfinding']['concluded'] == True:
            return
        if self.movement_stats['pathfinding']['initiated'] == False:
...
#            line_x = create_line(self.fov_deltoid['origin']['pos'], positions[x])
    ...
    def determine_obstruction_movement_nodes(self, obscuring_unit_ids, nodal_dist):
        ...
        for x in range(len(obscuring_units)):
            obscuring_unit_x = {'nodes': [], 'node_stats': {}}
            [pygame.draw.circle(DISPSURF, (255, 0, 0), self.master_sector.units[obscuring_units[x]]['bounding_points'][i], 5, 0) for i in range(len(self.master_sector.units[obscuring_units[x]]['bounding_points']))]
            pygame.draw.polygon(DISPSURF, (0, 0, 0), [self.master_sector.units[obscuring_units[x]]['bounding_points'][0], self.master_sector.units[obscuring_units[x]]['bounding_points'][1], self.master_sector.units[obscuring_units[x]]['bounding_points'][3], self.master_sector.units[obscuring_units[x]]['bounding_points'][2], self.master_sector.units[obscuring_units[x]]['bounding_points'][0]], 1)
            for i in range(len(self.master_sector.units[obscuring_units[x]]['bounding_points'])):
                unobstructed_point = True
                bounding_i_angle = normalize_degree(self.master_sector.units[obscuring_units[x]]['degree_val'] + {0: -45.0, 1: 45.0, 2: -135.0, 3: 135.0}[i])
                new_bounding_i = vec2int(pos_from_degrees(self.master_sector.units[obscuring_units[x]]['bounding_points'][i], bounding_i_angle, node_creation_distance))
                pygame.draw.circle(DISPSURF, (86, 0, 255), new_bounding_i, 8, 3)
                new_bounding_rect_points = [(new_bounding_i[0] - int(self.tank.node_distance / 2), new_bounding_i[1] - int(self.tank.node_distance / 2)), (new_bounding_i[0] + int(self.tank.node_distance / 2), new_bounding_i[1] + int(self.tank.node_distance / 2))]
                new_bounding_i_rect = self.master_sector.create_rect_from_points(new_bounding_rect_points)
                sectors_occupied = self.master_sector.sector_point_collision_omni(new_bounding_i_rect)
    ...
    def create_nodal_bounding_box(self, node_a, node_b, bounding_dist):
        ...
        nodal_normalized_zero_index_deg = normalize_degree(angle_between_points(nodal_bounding_box[0], nodal_bounding_rect.center) - nodal_deg_val)
        nodal_bounding_zero_degs = [nodal_normalized_zero_index_deg, abs(nodal_normalized_zero_index_deg), polarize_degree(abs(nodal_normalized_zero_index_deg)), polarize_degree(nodal_normalized_zero_index_deg)]
    ...
    def check_nodal_bounding_box_collisions(self, nodal_bounding_stats, origin_units=[], *, check_units=False):
        '''
        ...
        if check_units == False:
            units_in_node_sectors = list({self.master_sector.sectors[node_sectors[x]].unit_list_omni[i] for x in range(len(node_sectors)) for i in range(len(self.master_sector.sectors[node_sectors[x]].unit_list_omni)) if self.master_sector.sectors[node_sectors[x]].unit_list_omni[i] != self.unit_id and self.master_sector.sectors[node_sectors[x]].unit_list_omni[i] not in origin_units and self.master_sector.units[self.master_sector.sectors[node_sectors[x]].unit_list_omni[i]]['unit_type'] == 'obstacle'})
        else:
            units_in_node_sectors = list({self.master_sector.sectors[node_sectors[x]].unit_list_omni[i] for x in range(len(node_sectors)) for i in range(len(self.master_sector.sectors[node_sectors[x]].unit_list_omni)) if self.master_sector.sectors[node_sectors[x]].unit_list_omni[i] != self.unit_id and self.master_sector.sectors[node_sectors[x]].unit_list_omni[i] not in origin_units})
        ...
        if traversable == True:
            box_color_chosen = (0, 0, 0)
        else:
            box_color_chosen = (255, 0, 0)
        for x in range(len(nodal_bounding_stats['bounding_points'])):
            pygame.draw.circle(DISPSURF, box_color_chosen, nodal_bounding_stats['bounding_points'][x], 5, 0)
            pygame.draw.aaline(DISPSURF, box_color_chosen, nodal_bounding_stats['bounding_points'][x], nodal_bounding_stats['bounding_points'][nodal_bounding_stats['bounding_connections'][x]['clockwise_index']])
            pygame.draw.aaline(DISPSURF, box_color_chosen, nodal_bounding_stats['bounding_points'][x], nodal_bounding_stats['bounding_points'][nodal_bounding_stats['bounding_connections'][x]['counterclock_index']])
    ...
    def determine_nodal_movement_path(self, obscuring_units_path_points, goal_rect):
        ...
        if len(omni_sighted_units) > 0:
            for x in range(len(omni_sighted_units)):
                ...
                path_results = self.provide_path_from_node_pool(path_start_node, path_goal_nodes, obscuring_units_path_points[omni_sighted_units[x]]['node_stats'], self.movement_stats['pathfinding']['goal_node'])
        ...
        if valid_path_found == False:
            ...
            if len(artificial_to_goal_nodal_bridges['bridge_paths']) > 0:
                for x in range(len(artificial_to_goal_nodal_bridges['bridge_paths'])):
                    if artificial_path_identified != None:
                        break
                    ...
                    for i in range(len(artificial_to_goal_nodal_bridges['bridge_paths'][x])):
                        ...
                        path_results = self.provide_path_from_node_pool(current_obs_start_node, next_obs_goal_nodes, obscuring_units_path_points[current_unit]['node_stats'], self.movement_stats['pathfinding']['goal_node'])
                        if path_results != None:
                            if i == 0:
    ...
    def provide_path_from_node_pool(self, start_node, goal_node_list, node_stats, goal_node_pos=None):
        '''
        ...
        for x in range(len(directions)):
            ...
            if current_node in goal_node_list:
                ...
                if goal_node_pos != None:
                    current_path['path'][current_node] = goal_node_pos
            ...
            while pathfinding:
                ...
                if next_node in goal_node_list:
                    ...
                    current_path['valid'] = True
                    if goal_node_pos != None:
                        current_path['path'][next_node] = goal_node_pos
                    continue
                current_node = next_node
    ...
    def general_goal_pathfinding(self, goal_pos, artificial_goal_bounding):
        '''
        A low-cost, general pathfinding methodology utilized as a means of avoiding the more accurate yet intensive pathfinding algorithms.
        '''
    ...
    def draw_pathfinding_path(self):
        if self.movement_stats['pathfinding']['path_found'] == False:
            return
        path_color = (86, 0, 255)
        node_color = (0, 255, 0)
        path_list = []
        path_key = 'start_node'
        for _ in range(len(list(self.movement_stats['pathfinding']['path'].keys()))):
            path_list.append(self.movement_stats['pathfinding']['path'][path_key])
            path_key = self.movement_stats['pathfinding']['path'][path_key]
        print(path_list)
        if len(path_list) <= 1:
            return
        ...
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYUP and event.key == K_ESCAPE):
                ...
                if (event.key == K_LEFT or event.key == K_a):
                    ...
                    artificial_enemy.reset_movement_stats()
                    artificial_enemy.movement_stats['pathfinding']['goal_node'] = (mouse['mouse_x'], mouse['mouse_y'])
                    artificial_enemy.movement_stats['pathfinding']['clearance'] = 2
                elif (event.key == K_u):
